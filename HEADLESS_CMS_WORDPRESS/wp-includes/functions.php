...
add_action('rest_api_init', function () {
    register_rest_route('my_route', '/my_route/(?P<parameter>\d+)', array(
            'methods' => 'GET',
...
function my_function($data) {
    $parameter_value = $data['parameter'];
    return new WP_REST_Response($parameter_value, 200);
}

add_action('rest_api_init', function () {
    register_rest_route('my_route', '/database_route/', array(
            'methods' => 'GET',
            'callback' => 'database_function',
    ));
});
...
function database_function() {
    ...
    $username = "root";
    $password = "";
    $db = "headless_cms_wordpress";
    $connect = mysqli_connect($hostname,$username,$password,$db);
    if ($connect -> connect_error) {
        return new WP_REST_Response('connection failed', 400);
    }
    $query = mysqli_query($connect, 'SELECT * FROM wp_headless_cms_wordpressusers');
    ...
    return new WP_REST_Response($json_result, 200);
}
